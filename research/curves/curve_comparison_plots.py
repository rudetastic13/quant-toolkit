...
from more_interp_curve import Curve, InterpType, Term, TermUnit

# ---------------------------------------------------------------------------
# Shared node inputs  (np.ndarray[datetime64[D]] + np.ndarray[float64])
# ---------------------------------------------------------------------------

NODE_DATES: NDArray[np.datetime64] = np.array([
...
NODE_VALUES: NDArray[np.float64] = np.array(
...
PILLAR_DATES: NDArray[np.datetime64] = NODE_DATES[1:]   # exclude origin

CUTOVER_TERM = Term(2, TermUnit.YEARS)   # always 2-year cutover

# ---------------------------------------------------------------------------
# Cutover combinations to compare  (short-end method, long-end method)
# ---------------------------------------------------------------------------

CUTOVER_PAIRS: list[tuple[InterpType, InterpType]] = [
    (InterpType.DF_LOG_LINEAR, InterpType.DF_LOG_CUBIC),
    (InterpType.RATE_LINEAR,   InterpType.RATE_QUADRATIC),
    (InterpType.RATE_LINEAR,   InterpType.RATE_CUBIC),
]
...
def _label(itype: InterpType) -> str:
    return itype.value.replace("_", " ").title()


def _cutover_label(short: InterpType, long: InterpType) -> str:
...
def plot_single_interpolations(save_path: str | None = None) -> plt.Figure:
    ...
    ref_curve = Curve(NODE_DATES, NODE_VALUES, InterpType.DF_LOG_LINEAR)
    t_pil, df_pil, r_pil = _pillar_values(ref_curve)

    for idx, itype in enumerate(InterpType):
        curve = Curve(NODE_DATES, NODE_VALUES, itype)
